...
> **代码修订**：本仓库当前为 `core-75c2b55c…`；已发表归档每一行记录的是
> `core-a8deebd9…`，`core-f018acb5…` 是同一份代码去掉行尾差异后的形式。
> 三者只差**标识符命名与行尾**，数学完全一致 —— 见第六节「代码修订标识」的核对方法。
> 本仓库是这份代码的干净子集，去掉了 1.7 GB 历史输出、第三方上游代码树与服务器备份。
...
| 取值 | 规则 | 它标识什么 |
|---|---|---|
| `core-a8deebd9ddef8772f9768affdf9ee483d1e48193` | 逐字节，部署时的原样 | 已发表归档的每一行记录的值 |
| `core-f018acb54d16e1cfd42e539d86562dcff1577365` | 归一化行尾 | 同一份代码，但**改名之前** |
| `core-75c2b55cf133a5bf762f4277b252b8c7977da757` | 归一化行尾 | **本仓库**当前的代码 |

前两者描述同一份代码：部署时的树恰好是混行尾
（`task_path_fortification.py` 为 CRLF、`rev/panels.py` 为 LF），这是唯一的差异来源。
用归一化规则分别对**部署树、改名前的发布树、全新 clone** 求哈希，三者一致 ——
复现性断言就是这样核对过的。

第三个与第二个**只差标识符名字** —— 即 `MPCF` 改名，以及随之改变的
`NUMERICAL_CORE` 文件名（如 `mpcf_supplementary.py`）。
没有任何公式、界、代价系统或面板定义发生变化。这一点是用最强的方式验证的：
把已发表归档的 3 834 个 method 标签改成 `MPCF` 名后重跑全部下游环节，
**34 个统计与表格 CSV 的数值完全不变**。

所以 clone 得到的是 git SHA，非检出副本得到 `core-75c2b55c…`；
两种情况下 `code_revision_consistency` 门都成立，因为一次运行内所有行取值相同。
拿新跑的结果与已发表 CSV 对照时，请比**数字**，而不是比这个标识符。
