...
**超时运行记录的是进程被停止时的墙钟，不是求解器需要的时间。** 那 116 条超时
记录的取值有 112 个不同值、**没有一条恰好等于 3600.0**（范围 3600.016–3666.293 s），
所以它们是真实耗时，但对"求解速度"而言是**右截断**测量。

因此图 5 与表格必须分开呈现两类值，本版本已如此处理：

| 列 | 含义 |
|---|---|
| `runtime_selection_median_s` | 全部运行的中位（含被截断的） |
| `runtime_selection_median_completed_s` | **仅正常完成**运行的中位 |
| `time_limit_count` | 被停止的运行数 |
| `completed_count` | 正常完成的运行数 |

图 5 中，实线是"仅正常完成"的中位；**空心标记落在预算线上，表示该规模有运行被停止**，
不是它的完成时间。

### Greedy 的 0/9 不是性能结论

`MPCF-Greedy` 的 `solved_count` 恒为 0 是**输出契约**：代码对 Greedy 固定
`solver_optimal = False`（`harness.py`），它本来就不提供全局最优性证明。
**0/9 反映的是输出机制，不代表它得到的解非最优。** 事实上 Greedy 在 scaling 面板中
有 **21 条结果与已知最优值相等**（全部在 N=200/300）。

应分开报告：

| 指标 | Exact / CG | Greedy |
|---|---|---|
| 自证最优比例 | `solved_count` / 9 | 不适用（恒 0，设计使然） |
| 正常完成比例 | `completed_count` / 9 | `completed_count` / 9 |
| 达到已知最优值 | — | `at_known_optimum_count` |
| 限时输出质量 | — | 见下 |

### N=1000 下 Greedy 返回的是空集

N=1000 的三档预算共 **27 次** Greedy 运行，**全部**记录
`selected_count = 0`、`actual_cost = 0`、`status = time_limit`。当前实现需要完成
一整轮候选评估才提交下一次选择；中途超时则返回此前已提交的集合，也就是空集。
所以这些记录的 `kappa` 是 **κ(∅)（空加固集的值）**，**不是 Greedy 算法的求解质量**，
更不能据此推断"给更多时间也没用"。（N=500 每档 9 次里有 3 次是空集，其余提交了部分选择。）

### Greedy 差距：全实例区间，而非已知最优子集

`relative_gap` 需要 `kappa_opt`，而 `kappa_opt` 只在 Exact **或** CG 关证时才存在
（`harness.certify_cells` 同时接受两者的 `solver_closed` / `cg_closed`）。
仅用这个子集会在 N=1000 @ 10% 严重误导。

对实例 $i$，设 Greedy 结果为 $G_i$，且 $L_i \le \kappa_i^* \le U_i$
（$L_i$ 取各方法的可行解值与 `cg_L`，$U_i$ 取 `best_bound` / `cg_U` 与 `kappa_opt`），
由于 $g_i = 1 - G_i/\kappa_i^*$ 关于 $\kappa_i^*$ 单调递增：

$$1 - \frac{G_i}{L_i} \;\le\; g_i \;\le\; 1 - \frac{G_i}{U_i}$$

中位数单调，故**中位差距被夹在"下界的中位"与"上界的中位"之间**。
这是优化界，不是置信区间。

| N | 预算 | 已知最优覆盖 | **全实例中位差距区间** | 仅已知最优子集 | 空集输出 |
|---|---:|---:|---|---:|---:|
| 200 | 2% | 9/9 | 0.0000% | 0.0000% | 0 |
| 200 | 5% | 9/9 | 2.8037% | 2.8037% | 0 |
| 200 | 10% | 9/9 | 7.5000% | 7.5000% | 0 |
| 300 | 2% | 9/9 | 2.7027% | 2.7027% | 0 |
| 300 | 5% | 9/9 | 10.1266% | 10.1266% | 0 |
| 300 | 10% | 9/9 | 18.7500% | 18.7500% | 0 |
| 500 | 2% | 9/9 | 10.2564% | 10.2564% | 3 |
| 500 | 5% | 9/9 | 18.2482% | 18.2482% | 3 |
| 500 | 10% | 5/9 | 25.5738%（恰为精确） | 23.3333% | 3 |
| 1000 | 2% | 9/9 | 10.8481% | 10.8481% | 9 |
| 1000 | 5% | 8/9 | 20.2822%（恰为精确） | 20.5389% | 9 |
| 1000 | 10% | 4/9 | **[28.9308%, 29.0036%]** | 50.0000% | 9 |

> **更正**：本摘要早前写「N=1000 @ 10% 中位 gap = 0.500」。那是**仅 4 个已知最优实例**
> 的中位；**全部 9 个实例**的中位是 **28.9308%–29.0036%**。
> 换句话说，只报已知最优子集是**高估**该格约 21 个百分点，而不是低估 ——
> 该格里恰好是有最优值的实例差距最大（3 个 centralized 实例都是精确的 50%）。
> 覆盖不足的规模—预算组合共 **3 个**（N=500@10%、N=1000@5%、N=1000@10%）。
>
> 完整数据见 `tables/table_scaling_gap_bounds.csv`。
